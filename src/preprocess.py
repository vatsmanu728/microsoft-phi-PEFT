...
Converts the raw (article, highlights) DatasetDict from data_loader.py into
(prompt, target) pairs ready for tokenization:
  - prompt: config['instruction_template'] with the article inserted
  - target: the reference summary (highlights)

NOTE: this module does NOT apply any chat-template special tokens
(<|user|>, <|assistant|>, etc.) — that happens in tokenizer.py via
tokenizer.apply_chat_template(), using config['system_prompt'] and the
'prompt' field produced here as the user turn. Keeping this separation
means preprocess.py has no model-specific formatting logic at all.
"""
...
def _clean_article_text(text: str) -> str:
    ...
    Does NOT truncate length — truncation to max_seq_length happens later,
    in tokenizer.py, where it can be done in token space rather than
    character space.
    """
...
def preprocess_datasets(datasets: DatasetDict) -> DatasetDict:
    ...
    (columns: article, highlights) and returns a new DatasetDict with
